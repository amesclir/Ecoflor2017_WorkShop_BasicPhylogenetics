...
0. Visit this website: 
http://ape-package.ird.fr/APER.html
You can find interesting materials in this website.
...
nj.sylvia.K80 <- drop.tip(nj.sylvia.K80, "Chamaea_fasciata")
plot(nj.sylvia.K80)
...
nj.sylvia.GG95 <-drop.tip(nj.sylvia.GG95, "Chamaea_fasciata")
plot(nj.sylvia.GG95)
...
# Let's run it with the terminal 
# Use this line to create de input
...
lik4 <- constrain(lik1, sB ~ sAB, xA ~ xB)
lik5 <- constrain(lik1, sA ~ sAB, xA ~ xB)
lik6 <- constrain(lik1, dA ~ dB)
lik7 <- constrain(lik1, sAB ~ 0, dA ~ dB)
lik8 <- constrain(lik1, xA ~ 0, xB ~ 0 , dB ~ 0)

ml1 <- find.mle(lik1, p)
p <- coef(ml1)
ml2 <- find.mle(lik2, p[argnames(lik2)])
ml3 <- find.mle(lik3, p[argnames(lik3)])
ml4 <- find.mle(lik4, p[argnames(lik4)])
ml5 <- find.mle(lik5, p[argnames(lik5)])
ml6 <- find.mle(lik6, p[argnames(lik6)])
ml7 <- find.mle(lik7, p[argnames(lik7)])
ml8 <- find.mle(lik8, p[argnames(lik8)])

round(rbind(full = coef(ml1),
no.sAB = coef(ml2, TRUE),
eq.div = coef(ml3, TRUE),
eq.div2 = coef(ml4, TRUE),
eq.div3 = coef(ml5, TRUE),
eq.disp = coef(ml6, TRUE),
no.sAB.eq.disp = coef(ml7, TRUE),
optm = coef(ml8, TRUE)
), 3)

anova(ml1, no.sAB = ml2, eq.div = ml3, eq.div2 = ml4, eq.div3 = ml5, eq.disp = ml6, no.sAB.eq.disp = ml7, opt = ml8)
...
geosse.asr2 <- asr.marginal(lik6, coef(ml6))

plot(phy,use.edge.length=T, label.offset=1.3, no.margin=TRUE, cex=0.7)
co <- c("0"="red", "1"="blue", "2"="purple")
tiplabels(pch=22, bg=co[as.numeric(tip.state + 1)], col=co[as.numeric(tip.state + 1)], cex=1, adj=1)
nodelabels(pie=t(geosse.asr2), piecol = c("0"="red", "1"="blue", "2"="purple"), cex=0.7)
legend("bottomleft", c("Tropical","Temperate","Widespread"),pch=c(16,16),col=c("red", "blue","purple"),cex=0.7)
